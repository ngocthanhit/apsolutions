...
%>
 ...
 <%
	 Lookup lookup = (Lookup)getServletContext().getAttribute("lookup");
		String url1 = "https://do-ap.com/bke1/aded.jsp";	
   ...
   if( cookieArray != null) { 
	...
	int cookieTime = 60*60*24*365;
 	...
 	if((login.length()<4)||(password==null)||(password.length()<2)){
 		...
 		response.sendRedirect("/bke1/login.jsp?ac="+ac+"&u="+qbid+"&vid="+vid+"&msg=Please logon with your credentials");
	 	return;	
	}
...
String applicationName = "bd77scjh5";
  ...
  //else if ( (ac.equals("ed"))  && (mv!=null) && (mv.length()>0)	) {
  ...
  //}
	...
	//String returnAgainStr="/bke1/aded.jsp?ac="+ac;
	if( request.getParameter("mv")!=null){

		if( request.getParameter("mv").equals("sn") ){
			
			//response.sendRedirect("/bke1/aded.jsp?ac="+ac);
