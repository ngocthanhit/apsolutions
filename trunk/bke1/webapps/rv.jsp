   ...
   if( cookieArray != null) { 
	...
	int cookieTime = 60*60*24*365;
 	...
 	if((login.length()<4)||(password==null)||(password.length()<2)){
 		...
 		response.sendRedirect("/bke1/login.jsp?ac=rv&u="+qbid+"&vid="+vid+"&msg=Please logon with your credentials 1");
	 	return;	
	}
	...
	if((qbid.length()>0)&&(!qbid.equalsIgnoreCase(login))){	
		...
		response.sendRedirect("/bke1/login.jsp?ac=rv&u="+qbid+"&vid="+vid+"&msg=Please logon with your credentials 2");
 		return;	
	}
  ...
  System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ 222"); 	      
	  ...
	  if ( (mv!=null) &&   mv.equals("nb") ){
 		...
 		if(!qbid.equalsIgnoreCase(login)){
 			...
 			response.sendRedirect("/bke1/login.jsp?ac=rv&u="+qbid+"&vid="+vid+"&msg=Please logon with the same credentials that you used in Quick Base Application");
	 		return;	
		}
